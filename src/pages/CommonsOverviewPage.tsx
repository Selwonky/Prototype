  ...
  return (
    <>
      <PageHeader
        ...
        actions={<ButtonLink to="/functions" variant="outline">View 9 functions</ButtonLink>}
      />
      ...
      <GridPageShell columns={4} title="Commons objects">
        {commonsOverview.objects.map((object) => (
          <Card className="p-4" key={object}>
            ...
            <p className="mt-2 text-theme-xs text-gray-500 dark:text-gray-400">Shared object available across functions.</p>
          </Card>
        ))}
      </GridPageShell>
      ...
      <Section title="Interconnectivity">
        <Card>
          ...
          <CardContent className="grid gap-3 md:grid-cols-3">
            {joFunctions.map((fn) => (
              <Link className="rounded-lg border border-gray-200 p-3 transition-colors hover:bg-gray-50 dark:border-gray-800 dark:hover:bg-white/5" key={fn.id} to={`/functions/${fn.id}`}>
