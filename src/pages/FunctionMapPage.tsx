...
export function FunctionMapPage() {
  return (
    <>
      <PageHeader
        title="Jo from Workspaces"
        description="The 9 department workspaces and the Commons object layer that connects them."
      />
      ...
      <GridPageShell columns={3} title="9 Jo from workspaces">
        {joFunctions.map((fn) => {
          return (
            <Card className="grid min-h-56 grid-rows-[1fr_auto] p-0" key={fn.id} style={departmentSurfaceStyle(fn.id)}>
              <div className="min-h-0">
                <CardHeader>
                  <div className="space-y-3">
                    <div className="flex items-center justify-between gap-3">
                      <CardTitle>{fn.name}</CardTitle>
                      <Badge color="neutral" size="sm" variant="outline">{fn.id}</Badge>
                    </div>
